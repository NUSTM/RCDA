...
def get_rev_txtid(new_sentence, w2id, max_seq):
...
def get_data(path, w2id, ant_word_dic,max_seq):
    ...
    with open(path, encoding='utf-8') as f:
        for line in tqdm(f):
            ...
            rev_text_id = get_rev_txtid(new_sentence, w2id, max_seq)
            rev_text.append(rev_text_id)
    return src_text, rev_text, labels


class textDataset_genetate(Dataset):
    def __init__(self, path, w2id, ant_word_dic,max_seq):
        src_text, tgt_text, label = get_data(path, w2id,ant_word_dic,max_seq)
