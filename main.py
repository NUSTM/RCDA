...
def final_test(model, pres_rev, test_loader, threshold):
    ...
    pres_raw, pres_rev, labels = torch.tensor(pres_raw).cuda(), torch.tensor(pres_rev).cuda(), torch.tensor(
        labels).cuda()
...
def ant_rl_train(lm_model, ori_model, rev_model, opt_rev_cls, sample_nums, opt, vocab_size, train_loader, val_loader,
                 test_loader):
    step = 0
        ...
        for src, rev_tgt, label in tqdm(train_loader):
            ...
            for i in range(sample_nums):
                ...
                if config.val_max_acc > 0.75 and config.num_labels == 2 or config.val_max_acc > 0.35 and config.num_labels == 5:
                    ...
                    new_confids.append(
                        torch.abs(ori_model(temp)[0].softmax(-1).detach() - one_hot).sum(-1).unsqueeze(
                            -1))  # sample_num *batch*1
...
def train():
    ...
    if not os.path.exists(config.rev_pred_path):
        ...
        ant_rl_train(lm_model, ori_model, rev_model, rev_optimizer, config.m, opt_lm, vocab_size, rev_train_loader,
                     rev_val_loader, rev_test_loader)
