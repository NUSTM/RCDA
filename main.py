...
def get_loader(data_paths, w2id, ant_word_dic,batch_size):
    path=config.ant_dataset

    if not os.path.exists(path):
        datasets = []
        for data_path in data_paths:
            dataset = textDataset_genetate(data_path, w2id,ant_word_dic, config.max_seqlen,config.k)
    ...
    train_loader = DataLoader(datasets[0], batch_size=batch_size, shuffle=True, drop_last=False)
    val_loader = DataLoader(datasets[1], batch_size=batch_size, shuffle=False, drop_last=False)
    test_loader = DataLoader(datasets[2], batch_size=batch_size, shuffle=False, drop_last=False)
...
def final_test(model,pres_rev,test_loader,threshold):
    ...
    pres_raw,pres_rev,labels=torch.tensor(pres_raw).cuda().softmax(-1),torch.tensor(pres_rev).cuda().softmax(-1),torch.tensor(labels).cuda()
...
def train():
    ...
    if not os.path.exists(config.ant_dic_path) or not os.path.exists(config.vocab_path):
        ...
        get_word_dic(config.data_path, config.ant_dic_path,config.vocab_path,config.k)
    ...
    rev_train_loader, rev_val_loader, rev_test_loader = get_loader(
        [config.train_path, config.val_path, config.test_path], w2id,ant_dic,config.batch_size)
    ...
    rev_train_loader, rev_val_loader, rev_test_loader = get_loader(
        [config.train_path, config.val_path, config.test_path], w2id,ant_dic,config.rl_batch_size)
